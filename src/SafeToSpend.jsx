...
export default function SafeToSpend() {
  ...
  /* greyed example, hidden while the field has focus */
  const placeholderFor = useCallback(
    (k) =>
      focusedField === k ? "" : fmt(parse(DEMO_VALUES[currency][k] ?? "0")),
    [focusedField, currency, fmt, parse]
  );
  ...
  const loadDemo = () => { setV(DEMO_VALUES[currency]); setFreq(DEFAULT_FREQ); };
  ...
  const changeCurrency = (next) => {
    setV((prev) =>
      sameValues(prev, DEMO_VALUES[currency]) ? DEMO_VALUES[next] : prev
    );
    setCurrency(next);
  };
