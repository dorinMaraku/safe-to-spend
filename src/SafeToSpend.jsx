...
/* Sample figures for a small Tirana distributor, used to prefill the form
   during testing. Set PREFILL_DEMO to false before going live. */
const PREFILL_DEMO = true;
...
const DEMO_VALUES = {
  cash: "320,000", bank: "1,450,000", collections: "2,100,000",
  payroll: "780,000", taxes: "260,000", loans: "145,000",
  rent: "180,000", suppliers: "950,000", other: "120,000",
  reserve: "400,000",
};
...
const T = {
  sq: {
    ...
    cash: "Para në arkë",
    bank: "Para në bankë",
    collections: "Arkëtime të pritshme nga klientët",
    reliability: "Sa prej tyre i llogaritni si të sigurta?",
    reliabilityHelp:
      "Zgjidhni sa nga arkëtimet e pritshme i konsideroni të sigurta brenda periudhës së zgjedhur. Nëse nuk jeni të sigurt, mbajeni të ulët.",
    ...
    empty: "Filloni duke futur paratë që keni.",
...
function MoneyRow({
  id, label, value, onChange, onBlur, currency,
...
function Row({ label, value, bold, sub, color }) {
  return (
    <div
      ...
      style={{ fontSize: sub ? "0.72rem" : "0.8rem" }}
    >
...
export default function SafeToSpend() {
  /* NOTE FOR DEPLOYMENT ------------------------------------------------
     Browser storage is unavailable inside this preview, so nothing here
     persists. When hosting for real, seed lang / currency / reliability
     from localStorage and write back on change. Settings only — never
     the financial figures, or the privacy statement stops being true.
  ------------------------------------------------------------------- */
  ...
  const [v, setV] = useState(PREFILL_DEMO ? DEMO_VALUES : EMPTY_VALUES);
  ...
  const handleChange = useCallback(
    (k) => (e) => {
      ...
      if (/^[0-9.,\s-]*$/.test(raw)) setV((p) => ({ ...p, [k]: raw }));
    },
    []
  );
  ...
  const reset = () => { setV(EMPTY_VALUES); setFreq(DEFAULT_FREQ); };
  const loadDemo = () => { setV(DEMO_VALUES); setFreq(DEFAULT_FREQ); };
      ...
      >
        ...
        <div style={{ background: C.pineDeep }} className="px-4 pt-5 pb-4">
          <div className="flex items-center justify-between gap-3 mb-4">
            <span
              className="text-sm"
              style={{ color: "#fff", fontFamily: F.display, fontWeight: 600, fontSize: "1.05rem" }}
            >
              {t.appName}
            </span>
            <div className="flex gap-1.5">
              {[
                ...
                { v: currency, set: setCurrency, opts: [["ALL", "ALL"], ["EUR", "EUR"]] },
              ].map((grp, gi) => (
          ...
          {calc.started ? (
            <>
              <div className="flex items-baseline gap-2 flex-wrap">
                ...
                <span style={{ fontFamily: F.mono, color: "rgba(255,255,255,0.6)", fontSize: "0.85rem" }}>
                  {currency}
                </span>
              </div>
        ...
        {/* ------------------ content ------------------ */}
        <div className="flex-1 app-scroll px-3 pt-4" style={{ paddingBottom: 96 }}>
          {tab === 0 && (
              ...
              <Card title={t.inflowsTitle} note={t.inflowsNote}>
                ...
                <div className="px-4 py-4">
                  <div className="flex items-baseline justify-between gap-3 mb-2">
                    ...
                    <span style={{ fontFamily: F.mono, fontSize: "1.1rem", color: C.pine, fontWeight: 500 }}>
                      {reliability}%
                    </span>
                  </div>
                  <input
                ...
                <>
                  <Card title={t.stackTitle}>
                    <div className="px-4 pb-4 flex gap-3">
                      <div
                        className="w-8 rounded-md overflow-hidden flex flex-col shrink-0"
                        style={{ height: 220, background: "#EAEEEA" }}
                      >
                        {stack.map((s) => (
                          <div
                            key={s.key}
                        ...
                        {stack.map((s) => (
                          <div key={s.key} className="flex items-center justify-between gap-2">
                            <div className="flex items-center gap-2 min-w-0">
                              ...
                              <span className="text-xs truncate" style={{ color: C.inkSoft }}>{s.label}</span>
                            </div>
                            <span className="text-xs shrink-0" style={{ fontFamily: F.mono, color: C.muted }}>
                              {fmt(s.amount)}
                            </span>
                          </div>
                        ))}
                  ...
                  <div className="flex items-center gap-4 px-1 mb-3">
                    <button type="button" onClick={reset} className="text-xs"
                      style={{ color: C.muted, textDecoration: "underline" }}>
                      {t.reset}
                    </button>
                    <button type="button" onClick={loadDemo} className="text-xs"
                      style={{ color: C.muted, textDecoration: "underline" }}>
                      {t.demo}
                    </button>
                  </div>
